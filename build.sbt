...
libraryDependencies ++= Seq(
  ...
  "com.amazonaws"                % "aws-java-sdk-ses"                % versions.awsSES,
...
bashScriptExtraDefines ++= Seq(
  ...
  """addApp "-swagger.docs.endpoint=${SWAGGER_DOC_PATH:-/contactusservice/docs}"""",
...
defaultLinuxInstallLocation in Docker := "/opt/comtact-us-service"
packageName in Docker := "vr/comtact-us-service"
dockerBaseImage := "openjdk:8-jre-slim"
version in Docker := s"${if (gitHeadCode.value != "na") s"${version.value}_${gitHeadCode.value}" else version.value}"
maintainer in Docker := "Chenghsien Wen <chenghsien_wen@htc.com>"
...
dockerCommands := dockerCommands.value.take(1) ++ Seq(
  ...
  Cmd("LABEL", "owner_team=ad"),
  Cmd("ENV", "DOCKER_CONTENT_TRUST=1", "SERVICE_NAME=comtact-us-service SERVICE_TAGS=vr,service")
